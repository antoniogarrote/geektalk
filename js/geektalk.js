(function(){
	...
	var uri = "https://api.github.com/repos/"+viewModel.projectName();
	geektalk.loadJSONP(uri,function(resp) {
		...
		var uri = "https://api.github.com/repos/"+viewModel.projectName()+"?callback=jsonp";
	...
	var uri = "https://api.github.com/repos/"+viewModel.projectName()+"/collaborators?per_page=100&page="+page;
	geektalk.loadJSONP(uri,function(resp) {
	    if(resp != null && resp.data != null) {
		viewModel.projectContributorsFound(resp.data.length);
		var nextCollaborator = resp.data.pop();
		viewModel.pendingCollaborators(resp.data);
	...
	var uri = "http://github.com/api/v2/json/commits/list/"+viewModel.projectName()+"/master";
	var committers = [];
	var commits, author;
	...
	geektalk.loadJSONP(uri,function(resp) {
		...
		geektalk.resolveAndLoad("https://api.github.com/repos/*/*/collaborators", githubUser,function(success, jsonld){

		    // look for the collborator in Stack Overflow
		    var uri = "http://api.stackoverflow.com/1.1/users?filter="+login+"&key="+geektalk.stack_overflow_key;
			...
			var foundUser = geektalk.checkIdentity(originalGithubUser, jsonpData.users);
	...
	if(geektalk.pendingStackOverflowUsers.length > 0) {
	    ...
	    var uri = "http://api.stackoverflow.com/1.1/users/"+ids+"/answers?body=true&pagesize=20&order=desc&sort=votes&fromdate="+fromDate+"&key="+geektalk.stack_overflow_key;
	    viewModel.stackOverflowPagesLoaded++;

	    geektalk.loadJSONP(uri,function(jsonpData){
		var questionIds = [];
		for(var i=0; i<jsonpData.answers.length; i++) {
		    questionIds.push(jsonpData.answers[i].question_id);
		}
		

		geektalk.resolveAndLoad(uri, jsonpData.answers, function(success, jsonld){
		    if(success) {
			uri = "http://api.stackoverflow.com/1.1/questions/"+encodeURIComponent(questionIds.join(";"))+"?body=true&key="+geektalk.stack_overflow_key;		    
			geektalk.loadJSONP(uri,function(jsonpData){
			    geektalk.resolveAndLoad(uri, jsonpData.questions, function(success, jsonld){
	...
	var uri = "http://api.twitter.com/1/users/show.json?screen_name="+login;
	geektalk.loadJSONP(uri, function(data) {
    ...
    geektalk.buildVisualizations = function() {
        ...
        sko.store.execute(nodesQuery, function(success, nodes) {
            ...
            sko.store.execute(queryLinksQuery, function(success, links) {
                ...
                var node = vis.selectAll("circle.node")
                    ...
                    .attr("r", 5)
                    .style("fill", function(d) { return fill(d.node.group); })
                    .call(force.drag);
        ...
        sko.store.execute("SELECT ?tag WHERE { ?s so:tags ?tag }", function(success, bindings){
            ...
            chart.selectAll("rect")
               ...
               .enter().append("rect")
                 ...
                 .attr("height", 20)
                 .style("stroke","white")
                 .style("fill", function(d){ return geektalk.viewModel.colorForTag(d.name); })
            ...
            chart.selectAll("text.end_label")
               ...
               .append("text")
                ...
                .attr("x", function(d){ return 140+x(d.size)})
                .attr("y", function(d,i) { return i * 20; })
            ...
            div.data([{name:'tags',children: data}]).selectAll("div")
                ...
                .enter().append("div")
                .attr("class", "cell")
                .style("background", function(d) { return d.children ? null : geektalk.viewModel.colorForTag(d.name) })
                .call(cell)
        ...
        sko.store.execute(query, function(success, results){
            ...
            if(acum.length>0) {
                ...
                var bubbles = bubble.nodes({children: acum}).filter(function(d) { return !d.children; });
                ...
                node.append("circle")
                    ...
                    .style("fill", function(d) { return fill(d.className); });

                node.append("text")
                    .attr("text-anchor", "middle")
                    .attr("dy", function(d){ return (d.r>40) ? "1.5em" : ".3em"; })
                    .style("fill","white")
                    .text(function(d) { return d.className.substring(0, d.r / 3); });

                node.append("image")
                    .attr("width", function(d){ return (d.r>40)? 32 : 0; })
                    .attr("height", function(d){ return (d.r>40)? 32 : 0; })
                    .attr("x","-16")
                    .attr("y","-32")
